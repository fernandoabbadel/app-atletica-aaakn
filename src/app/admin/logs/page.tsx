...
export default function AdminLogsPage() {
  ...
  useEffect(() => {
    ...
    const loadLogs = async () => {
      setLoading(true);
      try {
        const rows = await fetchAdminActivityLogs({ maxResults: MAX_LOG_RESULTS });
        if (!mounted) return;
        setLogs(rows);
      } catch (error: unknown) {
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans pb-10">
      ...
      <main className="p-6 space-y-6">
        <div className="flex gap-4">
          ...
          <button className="bg-zinc-900 border border-zinc-800 px-4 rounded-xl text-zinc-400 hover:text-white transition" aria-label="Filtro de logs">
            <Filter size={18} />
          </button>
        </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center border ${getColor(log.action)}`}>
                      {getIcon(log.action)}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-white text-sm">{log.userName || "Sistema"}</span>
                      ...
                      <p className="text-xs text-zinc-400 mt-0.5">{log.details || "Sem detalhes."}</p>
                    </div>
                  </div>
