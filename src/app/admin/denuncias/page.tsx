...
interface Report {
    ...
    originCollection?: string; // Para saber onde salvar a resposta (mock ou firebase)
}

// --- MOCKS (Mantidos para outras categorias) ---
const INITIAL_REPORTS: Report[] = [
    {
        id: "mock-1", autor: "João Silva", alvo: "Pedro Souza", categoria: "comunidade",
        motivo: "Discurso de Ódio", descricao: "O usuário xingou meu time no post.",
        data: "10/01/2026 - 14:30", status: "pendente", originCollection: "mock"
    },
    {
        id: "mock-2", autor: "Carlos Jr", categoria: "suporte",
        motivo: "Erro no Pagamento", descricao: "Tentei pagar o plano Lenda e deu erro.",
        data: "12/01/2026 - 10:00", status: "pendente", originCollection: "mock"
    }
];
...
export default function AdminDenunciaPage() {
  ...
  // Estados
  const [activeCategory, setActiveCategory] = useState<ReportCategory>("banidos"); // Começa em Banidos para facilitar
  ...
  const getCategoryIcon = (cat: string) => {
      switch(cat) {
          case 'banidos': return <Lock size={18}/>; // 🦈 Novo Ícone
    ...
    <div className="min-h-screen bg-[#050505] text-white pb-20 font-sans selection:bg-emerald-500">
      ...
      <header className="p-6 sticky top-0 z-30 bg-[#050505]/90 backdrop-blur-md border-b border-zinc-800">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div><h1 className="text-xl font-black uppercase flex items-center gap-2"><ShieldAlert className="text-red-500" /> Tribunal do Tubarão</h1><p className="text-[11px] text-zinc-500 font-bold">Gestão de Denúncias & Apelações</p></div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 mb-6">
            {(['banidos', 'comunidade', 'gym', 'suporte'] as const).map(cat => (
                ...
                >
                    {getCategoryIcon(cat)}
                    <span className="text-[9px] font-black uppercase mt-1 tracking-wider">{cat}</span>
                </button>
            ))}
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
            ...
            <div className="relative w-full md:w-64">
                ...
                <input type="text" placeholder="Filtrar..." className="w-full bg-zinc-900 border border-zinc-800 rounded-lg pl-9 pr-3 py-2.5 text-xs text-white outline-none focus:border-emerald-500" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </div>
        </div>
      </header>
                      ...
                      <div key={report.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 flex flex-col md:flex-row gap-4 hover:border-zinc-700 transition">
                          <div className="flex-1">
                              <div className="flex justify-between items-start mb-2">
                                  <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase border ${report.status === 'pendente' ? 'bg-red-900/20 text-red-500 border-red-900/30' : 'bg-emerald-900/20 text-emerald-500 border-emerald-900/30'}`}>{report.status}</span>
                          ...
                          <div className="flex items-center md:border-l border-zinc-800 md:pl-4">
                              <button onClick={() => handleOpenReport(report)} className={`w-full md:w-auto px-5 py-3 rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 transition ${report.status === 'pendente' ? 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg' : 'bg-zinc-800 hover:bg-zinc-700 text-zinc-300'}`}>
      ...
      {selectedReport && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 overflow-y-auto">
              <div className="bg-zinc-900 w-full max-w-lg rounded-3xl border border-zinc-800 p-6 shadow-2xl relative animate-in zoom-in-95">
                  ...
                  <h2 className="font-bold text-white text-xl mb-1 flex items-center gap-2">{selectedReport.status === 'pendente' ? "Resolver Caso" : "Detalhes do Caso"}</h2>
                  ...
                  <div className="mb-6">
                      <label className="label-admin flex items-center gap-2 mb-2"><Bell size={12} className="text-yellow-500"/> {selectedReport.status === 'pendente' ? "Enviar Resposta/Notificação" : "Resposta Enviada"}</label>
                      <textarea className="input-admin min-h-[100px]" placeholder="Digite sua resposta..." value={responseText} onChange={e => setResponseText(e.target.value)} />
                  </div>

                  <div className="flex justify-between items-center pt-4 border-t border-zinc-800">
                      <button onClick={() => handleDelete(selectedReport)} className="text-red-500 text-xs font-bold uppercase hover:bg-red-900/20 px-3 py-2 rounded-lg transition flex items-center gap-2"><Trash2 size={14}/> Excluir</button>
                      {selectedReport.status === 'pendente' ? (
                          <button onClick={handleResolve} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 px-6 rounded-xl flex items-center gap-2 text-xs uppercase shadow-lg"><CheckCircle size={16}/> Resolver</button>
                      ) : (
                          <button onClick={handleResolve} className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold py-3 px-6 rounded-xl flex items-center gap-2 text-xs uppercase border border-zinc-700"><Save size={16}/> Atualizar</button>
                      )}
                  </div>
              </div>
          </div>
