...
const INITIAL_PLANOS = [
    { 
        nome: "Bicho Solto", preco: "0,00", precoVal: 0, parcelamento: "Gratuito", descricao: "Acesso básico.", cor: "zinc", icon: "ghost", destaque: false, beneficios: ["Carteirinha Digital", "Acesso ao App"],
        xpMultiplier: 1.0, nivelPrioridade: 1, descontoLoja: 0 
    },
    { 
        nome: "Cardume Livre", preco: "75,00", precoVal: 75, parcelamento: "Semestral", descricao: "Economia inteligente.", cor: "emerald", icon: "fish", destaque: false, beneficios: ["Kit: Caneca + Tirante", "10% bônus de XP"],
        xpMultiplier: 1.1, nivelPrioridade: 2, descontoLoja: 10 
    },
    { 
        nome: "Atleta de Bar", preco: "160,00", precoVal: 160, parcelamento: "2x Sem Juros", descricao: "O equilíbrio perfeito.", cor: "purple", icon: "star", destaque: true, beneficios: ["Kit Completo", "25% bônus de XP"],
        xpMultiplier: 1.25, nivelPrioridade: 3, descontoLoja: 15 
    },
    { 
        nome: "Lenda dos Jogos", preco: "250,00", precoVal: 250, parcelamento: "3x Sem Juros", descricao: "A elite do oceano.", cor: "yellow", icon: "crown", destaque: false, beneficios: ["VIP em Festas", "50% bônus de XP", "Destaque Dourado"],
        xpMultiplier: 1.5, nivelPrioridade: 4, descontoLoja: 20 
    }
];
...
export default function AdminPlanosPage() {
  ...
  const getTierFromPlan = (planName: string, planIcon: string): 'lenda' | 'atleta' | 'cardume' | 'bicho' => {
      ...
      if (lower.includes('lenda') || planIcon === 'crown') return 'lenda';
      if (lower.includes('atleta') || planIcon === 'star') return 'atleta';
      if (lower.includes('cardume') || planIcon === 'fish') return 'cardume';
      return 'bicho';
  };

  const handleApprove = async (sol: Solicitacao) => {
      if(!confirm(`Confirmar pagamento de ${sol.userName}? Isso atualizará o perfil dele em todo o app.`)) return;
      setIsSaving(true);
      ...
      try {
          ...
          const xpMult = planoRef?.xpMultiplier || 1.0; 
          const prioridade = planoRef?.nivelPrioridade || 1; 
  ...
  const handleCreate = () => { 
      setEditingPlan({ 
          id: "", nome: "", preco: "", precoVal: 0, parcelamento: "", descricao: "", 
          cor: "zinc", icon: "star", destaque: false, beneficios: ["Novo Benefício"],
          xpMultiplier: 1.0, nivelPrioridade: 1, descontoLoja: 0 
      }); 
      setIsModalOpen(true); 
  };
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans pb-20 selection:bg-emerald-500">
        <header className="p-6 sticky top-0 z-30 bg-[#050505]/90 backdrop-blur-md border-b border-white/5 shadow-lg flex justify-between items-center">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-xl font-black text-white uppercase tracking-tighter flex items-center gap-2"><CreditCard className="text-emerald-500" /> Central Financeira</h1>
            </div>
        ...
        <div className="px-6 pt-6">
            <div className="flex border-b border-zinc-800 gap-6 overflow-x-auto">
                ...
                <button onClick={() => setActiveTab("solicitacoes")} className={`pb-4 text-xs font-bold uppercase border-b-2 flex items-center gap-2 whitespace-nowrap transition ${activeTab === "solicitacoes" ? "text-emerald-500 border-emerald-500" : "text-zinc-500 border-transparent hover:text-white"}`}>
                    ...
                    {solicitacoes.filter(s => s.status === 'pendente').length > 0 && <span className="bg-red-500 text-white px-1.5 py-0.5 rounded-full text-[9px] animate-pulse">{solicitacoes.filter(s => s.status === 'pendente').length}</span>}
                </button>
            ...
            {activeTab === "dashboard" && (
                <div className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        {/* Outros Cards */}
                        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl flex items-center justify-between shadow-xl">
                            <div className="relative z-10">
                            ...
                            <div className="bg-blue-500/20 p-4 rounded-2xl text-blue-500"><Users size={32}/></div>
                        </div>
                        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl flex items-center justify-between shadow-xl">
                            <div className="relative z-10">
                            ...
                            <div className="bg-yellow-500/20 p-4 rounded-2xl text-yellow-500"><Crown size={32}/></div>
                        </div>
                    </div>
                                ...
                                <div key={plano.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 flex flex-col justify-between hover:border-zinc-700 transition">
                                    <div>
                                        ...
                                        <div className="flex gap-2 mt-2">
                                            <span className="text-[8px] bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-400 flex items-center gap-1"><TicketPercent size={8}/> -{plano.descontoLoja}%</span>
                                            <span className="text-[8px] bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-400 flex items-center gap-1"><ShieldAlert size={8}/> Lvl.{plano.nivelPrioridade}</span>
                                        </div>
                                    </div>
                                    <button onClick={() => setViewingPlanId(plano.id)} className="w-full mt-4 bg-black/40 hover:bg-black/60 text-zinc-400 hover:text-white border border-zinc-800 rounded-lg py-2 text-[10px] uppercase font-bold transition flex items-center justify-center gap-2"><Eye size={12}/> Ver Lista</button>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden shadow-xl">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left whitespace-nowrap">
                                <thead className="bg-black/40 text-zinc-500 text-[10px] font-bold uppercase"><tr><th className="p-4">Aluno</th><th className="p-4">Plano</th><th className="p-4">Valor</th><th className="p-4 text-right">Data Início</th></tr></thead>
                ...
                <>
                    <div className="flex justify-end mb-6">
                         <button onClick={handleSeedPlanos} disabled={isSaving} className="text-emerald-500 text-[10px] font-bold uppercase tracking-widest hover:underline flex items-center gap-2 bg-emerald-900/10 px-4 py-2 rounded-xl border border-emerald-900/30 hover:bg-emerald-900/20 transition">
                            {isSaving ? <Loader2 className="animate-spin" size={14}/> : <RefreshCw size={14}/>} Restaurar Padrões
                        </button>
                    </div>
                            ...
                            <div key={plano.id} className={`bg-zinc-900 border rounded-[2rem] p-6 flex flex-col relative overflow-hidden transition group hover:-translate-y-1 duration-300 ${plano.cor === 'yellow' ? 'border-yellow-500/30' : plano.cor === 'zinc' ? 'border-zinc-500/30' : 'border-emerald-500/30'}`}>
                                <div className="mb-4">
                                    ...
                                    <div className="mt-2 flex flex-wrap gap-2">
                                        <span className="text-[9px] font-bold bg-white/10 px-1.5 py-0.5 rounded text-white flex items-center gap-1"><Zap size={10}/> {plano.xpMultiplier}x XP</span>
                                        <span className="text-[9px] font-bold bg-white/10 px-1.5 py-0.5 rounded text-white flex items-center gap-1"><TicketPercent size={10}/> {plano.descontoLoja}% OFF</span>
                                        <span className="text-[9px] font-bold bg-white/10 px-1.5 py-0.5 rounded text-white flex items-center gap-1"><ShieldAlert size={10}/> Prio {plano.nivelPrioridade}</span>
                                    </div>
                                </div>
            ...
            <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/95 p-4 animate-in fade-in">
                <div className="bg-zinc-900 w-full max-w-md rounded-3xl border border-zinc-800 overflow-hidden flex flex-col max-h-[90vh] shadow-2xl">
                    <div className="p-4 bg-black/50 border-b border-zinc-800 flex justify-between items-center">
                        <div><h3 className="text-white font-bold uppercase text-sm">Comprovante de {viewingReceipt.userName}</h3><p className="text-zinc-500 text-[10px]">Valor: R$ {viewingReceipt.valor}</p></div>
                    ...
                    <div className="p-4 bg-zinc-900 border-t border-zinc-800 flex gap-3">
                        ...
                        <button onClick={() => handleApprove(viewingReceipt)} disabled={isSaving} className="flex-[2] bg-emerald-600 text-white hover:bg-emerald-500 py-3 rounded-xl font-bold uppercase text-xs transition shadow-lg flex items-center justify-center gap-2">{isSaving ? <Loader2 className="animate-spin" size={16}/> : <CheckCircle size={16}/>} Confirmar Pagamento</button>
                    </div>
                </div>
            </div>
        )}
        ...
        {/* 🦈 MODAL EDIÇÃO PLANO COM REGRAS DE NEGÓCIO */}
                ...
                <div className="bg-zinc-900 w-full max-w-2xl rounded-[2rem] border border-zinc-800 p-8 shadow-2xl relative my-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="bg-black/30 p-3 rounded-xl border border-zinc-800 space-y-3">
                                <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest mb-1 flex items-center gap-2"><Crown size={12}/> Regras de Negócio</p>
                                <div className="grid grid-cols-3 gap-2">
                                    <div>
                                        <label className="text-[9px] font-bold text-yellow-500 uppercase mb-1 block">XP Multiplier</label>
                                        <input type="number" step="0.1" className="input-admin text-center font-mono text-yellow-400 font-bold" value={editingPlan.xpMultiplier} onChange={e => setEditingPlan({...editingPlan, xpMultiplier: parseFloat(e.target.value)})}/>
                                    </div>
                                    <div>
                                        <label className="text-[9px] font-bold text-blue-500 uppercase mb-1 block">Prioridade (1-5)</label>
                                        <input type="number" min="1" max="10" className="input-admin text-center font-mono text-blue-400 font-bold" value={editingPlan.nivelPrioridade} onChange={e => setEditingPlan({...editingPlan, nivelPrioridade: parseInt(e.target.value)})}/>
                                    </div>
                                    <div>
                                        <label className="text-[9px] font-bold text-emerald-500 uppercase mb-1 block">Desc. Loja (%)</label>
                                        <input type="number" min="0" max="100" className="input-admin text-center font-mono text-emerald-400 font-bold" value={editingPlan.descontoLoja} onChange={e => setEditingPlan({...editingPlan, descontoLoja: parseInt(e.target.value)})}/>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <select className="input-admin" value={editingPlan.icon} onChange={e => setEditingPlan({...editingPlan, icon: e.target.value})}>
                                        <option value="ghost">Fantasma</option><option value="shopping">Bolsa</option><option value="star">Estrela</option><option value="crown">Coroa</option><option value="fish">Peixe</option><option value="zap">Raio</option><option value="gem">Diamante</option><option value="trophy">Troféu</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col h-full space-y-4">
                            <div><label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block">Descrição Curta</label><textarea rows={2} className="input-admin resize-none" value={editingPlan.descricao} onChange={e => setEditingPlan({...editingPlan, descricao: e.target.value})}/></div>
                            <div className="bg-black/40 p-5 rounded-2xl border border-zinc-800 flex flex-col flex-1">
                                <div className="flex justify-between items-center mb-4"><label className="text-[10px] font-bold text-zinc-500 uppercase">Benefícios Visuais</label><button onClick={addBenefit} className="text-[10px] bg-emerald-600 text-white px-3 py-1.5 rounded-lg hover:bg-emerald-500 font-bold shadow-lg transition">+ Adicionar</button></div>
                                <div className="space-y-3 overflow-y-auto pr-2 flex-1 custom-scrollbar max-h-[250px]">
