...
export default function ParceiroDetalhePage() {
  ...
  if (!parceiro) return <div className="text-white text-center py-20">Empresa não encontrada.</div>;
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-[2rem] p-6 shadow-2xl">
            ...
            <div className="space-y-3">
                {parceiro.cupons && parceiro.cupons.length > 0 ? parceiro.cupons.map((cupom: any) => (
                    ...
                    <p className="text-zinc-500 text-xs italic text-center py-4">Nenhum cupom disponível no momento.</p>
                )}
            </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/95 backdrop-blur-xl p-6 animate-in zoom-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[2rem] p-8 text-center relative shadow-2xl overflow-hidden">
            ...
            <div className="bg-zinc-100 rounded-xl p-4 mb-6 flex justify-between items-center border border-zinc-200">
                <div className="text-left"><p className="text-[10px] text-zinc-400 font-bold uppercase">Valor</p><span className="font-mono text-xl font-black text-black tracking-widest">{activeCupom.valor}</span></div>
                <button onClick={() => addToast("Mostre ao caixa!", "success")} className="bg-white p-2 rounded-lg border border-zinc-200 text-zinc-400 hover:text-emerald-600 shadow-sm"><CheckCircle size={20}/></button>
            </div>
