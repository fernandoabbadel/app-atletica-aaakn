...
// --- SUB-COMPONENTE INTERNO (CARD DARK) ---
...
export default function DashboardPage() {
  ...
  // Simulando Lista de Eventos
  const [events] = useState<Evento[]>([
    {
      id: '1',
      title: 'Intermed - O Início',
      category: 'Festa',
      lote: '1º Lote',
      date: '20/10 - 22h',
      location: 'Arena Sharks',
  ...
  // DADOS MOCKADOS EXTRAS (Adicione isso ao seu User interface no futuro)
  const turmaRanking = 6; // Exemplo: Sua turma é a 6ª colocada
  const matchesPlayed = user.dailyMatchesPlayed || 0;
  const remainingMatches = Math.max(0, 5 - matchesPlayed);

  // Define a foto da turma (Se estiver na pasta public, use o caminho relativo ex: '/turma_t5.jpg')
  // Se não tiver, usa um fallback online
  const classPhotoSrc = user.turmaPhoto || 'https://images.unsplash.com/photo-1523580494863-6f3031224c94';
  ...
  return (
    <div className="flex flex-col gap-6 p-4 pb-24 max-w-md mx-auto w-full bg-[#050505] min-h-screen text-white">
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <div className="h-14 w-14 rounded-full bg-gray-900 flex items-center justify-center border-2 border-orange-500 p-0.5 shadow-sm overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {/* BOTÃO GYMRATS (VERMELHO 🔴) */}
        <button className="group relative h-48 w-full overflow-hidden rounded-2xl bg-gray-900 shadow-xl transition-all active:scale-95 border border-red-900/30">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-2 text-white z-10">
            ...
            {/* INFO DA TURMA (Centralizado abaixo) */}
            <div className="mt-3 flex items-center gap-2 bg-black/40 px-3 py-1.5 rounded-full border border-red-500/30 shadow-lg backdrop-blur-sm">
                {/* Foto da Turma (Circulo Pequeno) */}
                <div className="h-6 w-6 rounded-full border border-red-500 overflow-hidden shrink-0">
                    <img src={classPhotoSrc} alt="Turma" className="w-full h-full object-cover" />
                </div>
                {/* Ranking da Turma */}
        ...
        {/* BOTÃO ARENA GAMES (ROXO 🟣) */}
        <button className="group relative h-48 w-full overflow-hidden rounded-2xl bg-gray-900 shadow-xl transition-all active:scale-95 border border-purple-500/30">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-2 text-white z-10">
            ...
            <div className="flex flex-col items-center mt-2">
              ...
              <span className="text-4xl font-black text-white drop-shadow-lg">{user.level}</span>
            </div>
