...
import { 
  Eye, EyeOff, Waves, Sparkles, Trophy, Users, 
  Calendar, ShoppingBag, CreditCard, Crown, ArrowRight, 
  MapPin, Mail, Phone, Instagram, ShieldCheck, Dumbbell, Star, Rocket
} from "lucide-react";
...
// --- COMPONENTE: Card de Estatística (Dados Reais) ---
const StatCard = ({ icon: Icon, value, label, color, suffix = "" }: any) => {
  const count = useCounter(value);
  return (
    <div className={`flex flex-col items-center p-4 rounded-2xl bg-zinc-900/60 border border-zinc-800 backdrop-blur-md hover:border-${color}-500/50 hover:shadow-[0_0_20px_rgba(var(--${color}-rgb),0.1)] transition-all group min-w-[110px]`}>
      <div className={`p-3 rounded-full bg-${color}-500/10 mb-3 group-hover:scale-110 group-hover:bg-${color}-500/20 transition duration-300`}>
...
// --- COMPONENTE: Card de Depoimento com Foto ---
...
export default function LandingPage() {
  ...
  const [stats, setStats] = useState({ users: 120, posts: 340, partners: 12 }); // Começa com valores fictícios base
  ...
  // 📡 Busca Dados Reais (Simulação Inteligente)
  useEffect(() => {
    const fetchStats = async () => {
        try {
            // Tenta buscar contagem real (se a coleção existir e tiver permissão)
            // Nota: getCountFromServer é mais barato que getDocs
            const usersColl = collection(db, "users");
            const snapshot = await getCountFromServer(usersColl);
            const realUsers = snapshot.data().count;

            // Se tiver usuários reais (>0), usa. Senão, mantém o fictício base pro visual não quebrar.
            if (realUsers > 0) {
                setStats(prev => ({ ...prev, users: realUsers }));
            }
        } catch (error) {
            console.log("Usando estatísticas base (DB ainda vazio ou offline).");
        }
    };
    fetchStats();
  }, []);
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-2xl mx-auto lg:mx-0">
                ...
                <div className="flex flex-col items-center p-3 rounded-2xl bg-gradient-to-b from-zinc-800 to-zinc-900 border border-amber-500/30 shadow-[0_0_15px_rgba(245,158,11,0.1)] relative overflow-hidden group">
                    ...
                    <div className="relative w-8 h-8 mb-2 rounded-full overflow-hidden border border-zinc-700 bg-zinc-800">
                        {/* Tente colocar uma logo em public/turmas/t5.png */}
                        <Image src="/turmas/t5.png" alt="Turma" fill className="object-cover opacity-80 group-hover:opacity-100 transition" />
