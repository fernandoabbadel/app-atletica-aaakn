...
const BADGES = [
  {
    ...
    icon: <GiSharkFin className="opacity-50" />,
  },
  {
    id: 2,
  ...
  },
  {
    ...
    icon: <GiSharkFin className="scale-125" />,
  },
  {
    id: 6,
    titulo: "MEGALODON",
...
export default function ConquistasPage() {
  ...
  const currentBadgeIndex = BADGES.slice()
    .reverse()
    .findIndex((b) => userStats.xp >= b.minXp);
  ...
  const conquistas = [
    ...
    {
      id: 6,
      titulo: "Monstro da Jaula",
      desc: "Check-in antes das 6h da manhã.",
      cat: "Gym",
      xp: 500,
      icon: <GiBiceps />,
      progress: 1,
    ...
    {
      id: 7,
      titulo: "Shape Inexplicável",
      desc: "100 Treinos confirmados.",
      cat: "Gym",
      xp: 2000,
      icon: <GiBiceps className="text-yellow-500" />,
      progress: 42,
      target: 100,
    },
    ...
    },
    {
      id: 10,
      titulo: "One Hit KO",
      desc: "Venceu com 1 golpe crítico.",
      cat: "Games",
      xp: 1000,
      icon: <GiBoxingGlove />,
      progress: 0,
    ...
    },
    {
      ...
      desc: "Foi em 1 evento acadêmico.",
      cat: "Eventos",
      xp: 100,
      icon: <GiOpenBook />,
      progress: 0,
    ...
    {
      id: 22,
      titulo: "Primeiro Lote",
      desc: "Comprou ingresso na hora.",
      cat: "Eventos",
      xp: 500,
      icon: <GiTicket />,
      progress: 1,
    ...
    {
      id: 24,
      titulo: "Tubarão Solidário",
      desc: "Doou para ação social.",
      cat: "Social",
      xp: 1000,
  ...
  const filteredConquistas =
    filtro === "Todas"
      ? conquistas
      : conquistas.filter((c) => c.cat === filtro);
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans pb-10 selection:bg-emerald-500/30">
      ...
      <header className="p-4 sticky top-0 z-20 bg-[#050505]/90 backdrop-blur-md flex items-center gap-3 border-b border-white/5 shadow-lg">
        ...
        <div className="flex-1">
          <h1 className="font-black text-lg italic uppercase tracking-tighter">
            Sala de Troféus
          </h1>
      ...
      <main className="p-4 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <section
          className={`relative overflow-hidden rounded-3xl border ${displayedBadge.border} ${displayedBadge.bg} p-6 text-center shadow-2xl transition-colors duration-500`}
        >
          {/* Setas de Navegação */}
          <button
            onClick={handlePrev}
          ...
          <div className="relative z-10 px-6">
            <div
              className={`mx-auto mb-4 flex h-28 w-28 items-center justify-center rounded-full bg-zinc-950/80 border-4 ${displayedBadge.border} shadow-[0_0_40px_rgba(0,0,0,0.3)] transition-all duration-500`}
            >
              <span
                className={`text-6xl drop-shadow-lg ${displayedBadge.cor} ${
            ...
            <h2
              className={`text-3xl font-black uppercase italic tracking-tighter ${displayedBadge.cor} drop-shadow-md transition-all duration-500`}
            >
            ...
            <div className="mt-2 min-h-[60px] flex flex-col items-center justify-center">
              {isCurrent && (
                <div className="w-full animate-in zoom-in duration-300">
                  <span className="text-[10px] font-bold text-white bg-emerald-500/20 px-3 py-1 rounded-full border border-emerald-500/30 uppercase tracking-widest mb-3 inline-block">
                    Patente Atual
                  </span>
        ...
        <section className="overflow-x-auto pb-2 scrollbar-hide">
          <div className="flex gap-2">
            {["Todas", "Gym", "Games", "Loja", "Eventos", "Social"].map(
              (cat) => (
                <button
                  key={cat}
          ...
          <div className="flex items-center justify-between px-1">
            ...
            <span className="text-[10px] text-zinc-400 font-bold bg-zinc-900 px-3 py-1 rounded-full border border-zinc-800">
              <span className="text-white">
                {userStats.conquistasDesbloqueadas}
              </span>{" "}
              / {userStats.totalConquistas} Liberadas
            </span>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 relative z-10">
                    {/* Ícone */}
                    <div
                      className={`h-14 w-14 shrink-0 rounded-2xl flex items-center justify-center border transition-colors ${
                        desbloqueada
                    ...
                    >
                      <span className="text-2xl">
                        {desbloqueada ? item.icon : <Lock size={20} />}
                      </span>
                    </div>
                      ...
                      <div className="flex justify-between items-start">
                        <h4
                          className={`text-sm font-bold truncate ${
                            desbloqueada ? "text-white" : "text-zinc-400"
                          }`}
                        >
                          {item.titulo}
                        </h4>
                        {desbloqueada && (
                          <CheckCircle2
                            size={16}
                            className="text-emerald-500 shrink-0 drop-shadow-glow"
                          />
                        )}
                      </div>
                      ...
                      <div className="mt-3 flex items-center gap-3">
                        <div className="h-2 flex-1 rounded-full bg-zinc-950 overflow-hidden border border-white/5">
                          <div
                            className={`h-full rounded-full ${
                              desbloqueada ? "bg-emerald-500" : "bg-zinc-700"
                            }`}
                            style={{ width: `${percent}%` }}
                          ></div>
                        </div>
                        <span
                          className={`text-[9px] font-black ${
                            desbloqueada ? "text-emerald-400" : "text-zinc-600"
                          }`}
                        >
