...
export default function AdminLayout({
  ...
  // Itens da Barra Lateral - Baseado na estrutura de pastas
  const sidebarItems = [
      ...
      <aside className="w-64 bg-zinc-900/90 backdrop-blur-xl border-r border-white/5 flex flex-col justify-between fixed h-full z-40 overflow-y-auto custom-scrollbar">
        <div className="p-6">
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h1 className="font-black text-white text-lg uppercase tracking-tighter leading-none">
                Painel Admin
              </h1>
          ...
          <div className="mb-6 p-3 bg-black/40 rounded-xl border border-zinc-800 flex items-center gap-3">
            ...
            <div className="overflow-hidden">
              ...
              <span className="text-[8px] font-black text-red-500 uppercase tracking-widest block truncate">
                {user?.role?.replace("admin_", "") || "MASTER"}
              </span>
            </div>
          </div>
          ...
          <nav className="space-y-1">
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest px-2 mb-2">
              Menu Principal
            </p>
        ...
        <div className="p-6 border-t border-zinc-800 bg-black/20">
          <a
            href="/configuracoes"
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-zinc-400 hover:bg-zinc-800 hover:text-white transition-all group mb-2"
          >
            <Settings size={18} />
            <span className="text-xs font-medium uppercase tracking-wide">
              Configurações
          ...
          <Link
            href="/"
            className="flex items-center justify-center gap-3 p-3 rounded-xl bg-red-600/10 text-red-500 border border-red-600/20 hover:bg-red-600 hover:text-white transition-all font-bold uppercase text-[10px] tracking-wider group"
          >
            <LogOut
              size={16}
